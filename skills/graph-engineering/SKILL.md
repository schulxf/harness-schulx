...
When the repository is managed by Harness Schulx, read
[references/harness-integration.md](references/harness-integration.md) before turning the graph
design into implementation work. Graph Engineering owns the graph model and task topology;
Harness owns the contract, execution state, deterministic evidence, and approval gates.
...
- [references/harness-integration.md](references/harness-integration.md) — Boundary and artifact
  contract for using this skill inside Harness Schulx. Read when a `.harness/` workflow or the
  `$harness-runner` skill controls execution.
...
This vendored copy comes from
[codejunkie99/graph-engineering](https://github.com/codejunkie99/graph-engineering) at commit
`cfacb56a05a31ba69bf84d0b8b00f5ce463127ef`. The upstream material is distributed under the
MIT License. Harness-specific guidance is isolated in `references/harness-integration.md` and
the routing note above.
